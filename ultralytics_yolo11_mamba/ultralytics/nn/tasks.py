...
from ultralytics.nn.modules.block import ExtractRGB, ExtractIR, IAGA_Module, Spatial_IAGA_Module
...
def parse_model(d, ch, verbose=True):
    ...
    # 【新增导入】：把我们写的三个新模块引进来
    from ultralytics.nn.modules.block import ExtractRGB, ExtractIR, IAGA_Module 
    ...
    base_modules = frozenset(
        {
            ...
            C2f_Mamba,
            DynamicSpatialFusion,
            ExtractRGB,  # <--- 【修改 1】：注册 RGB 提取分支
            ExtractIR    # <--- 【修改 2】：注册 IR 提取分支
        }
    )
    repeat_modules = frozenset(  # modules with 'repeat' arguments
        {
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c2 = sum(ch[x] for x in f)
        elif m in {IAGA_Module, Spatial_IAGA_Module}:  # <--- 改成集合匹配
            c1 = [ch[x] for x in f]      
            c2 = args[0]
            args = [c1, c2, *args[1:]]
        elif m in frozenset(
