...
class BaseDataset(Dataset):
    ...
    def load_image(self, i: int, rect_mode: bool = True) -> tuple[np.ndarray, tuple[int, int], tuple[int, int]]:
        ...
        if im is None:  # not cached in RAM
            ...
            # ========================================================
            # 【IAGA 四通道融合读取黑科技 (双模态动态拼接)】
            # ========================================================
            import cv2
            import numpy as np
            
            # 只有当图片是 3 通道时才进行拼接（防止读取到已经被拼接过并缓存的 npy 文件）
            if im.ndim == 3 and im.shape[-1] == 3:
                # 1. 强制将可见光从 BGR 转为 RGB！
                # 这是极其关键的一步。因为 YOLO 底层的增强脚本如果发现图片是 4 通道，
                # 就会跳过 BGR2RGB 的转换。我们必须在这里手动提前转好，否则预训练权重会崩。
                im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
                
                # 2. 自动推导 IR 图像路径 (兼容 Linux 的 / 和 Windows 的 \)
                f_str = str(f)
                f_ir = f_str.replace('/images/', '/images_ir/').replace('\\images\\', '\\images_ir\\')
                
                # 3. 读取伪三通道的 IR 图像
                im_ir = cv2.imread(f_ir)
                if im_ir is None:
                    raise FileNotFoundError(f"IR Image Not Found (未找到对应的红外图像): {f_ir}")
                
                # 4. 提取红外图像的单通道 (H, W, 3) -> (H, W, 1)
                im_ir_single = im_ir[:, :, 0:1]
                
                # 5. 在通道维度拼接：RGB (3) + IR (1) = 4 通道
                im = np.concatenate([im, im_ir_single], axis=-1)
