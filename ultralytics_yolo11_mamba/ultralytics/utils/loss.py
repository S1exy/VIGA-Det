...
def nwd_loss(bboxes1, bboxes2, eps=1e-7, constant=12.8):
    """
    计算两个边界框集合的 NWD Loss
    bboxes: (x_center, y_center, w, h)
    """
    # 提取中心点和宽高
    cx1, cy1, w1, h1 = bboxes1[..., :4].unbind(-1)
    cx2, cy2, w2, h2 = bboxes2[..., :4].unbind(-1)
    
    # 1. 计算中心点的欧氏距离平方
    center_dist = (cx1 - cx2) ** 2 + (cy1 - cy2) ** 2
    
    # 2. 计算宽高的 Wasserstein 惩罚项
    wh_dist = ((w1 - w2) ** 2 + (h1 - h2) ** 2) / 4.0
...
def rotated_nwd_loss(pred_bboxes, target_bboxes, constant=12.8, eps=1e-7):
    """
    基于旋转框高斯建模的 NWD Loss。
    pred_bboxes/target_bboxes: (N, 5) format: (cx, cy, w, h, theta)
    利用 _get_covariance_matrix 将 OBB 建模为 2D 高斯分布，计算 Wasserstein 距离。
    """
    from .metrics import _get_covariance_matrix

    # 中心点距离
    mu1 = pred_bboxes[..., :2]
    mu2 = target_bboxes[..., :2]
    center_dist = ((mu1 - mu2) ** 2).sum(-1)

    # 协方差矩阵 [[a, c], [c, b]] via Gaussian bounding box modeling
    a1, b1, c1 = _get_covariance_matrix(pred_bboxes)
    a2, b2, c2 = _get_covariance_matrix(target_bboxes)

    # Tr(Σ1 + Σ2)
    trace_sum = (a1 + b1 + a2 + b2).squeeze(-1)

    # det(Σ) = a*b - c^2
    det1 = (a1 * b1 - c1 ** 2).clamp(min=eps)
    det2 = (a2 * b2 - c2 ** 2).clamp(min=eps)

    # Tr(Σ1 Σ2) = a1*a2 + b1*b2 + 2*c1*c2
    trace_product = (a1 * a2 + b1 * b2 + 2 * c1 * c2).squeeze(-1)
    det_product = (det1 * det2).squeeze(-1)

    # Tr((Σ1^½ Σ2 Σ1^½)^½) = sqrt(Tr(Σ1 Σ2) + 2*sqrt(det(Σ1)*det(Σ2)))
    trace_sqrt_m = (trace_product + 2 * det_product.sqrt()).clamp(min=eps).sqrt()

    # W2² = ||μ1 - μ2||² + Tr(Σ1) + Tr(Σ2) - 2 * Tr((Σ1^½ Σ2 Σ1^½)^½)
    w2_sq = (center_dist + trace_sum - 2 * trace_sqrt_m).clamp(min=0)

    nwd = torch.exp(-w2_sq.sqrt() / constant)
    return 1.0 - nwd
...
import torch.nn as nn
import torch.nn.functional as F
# 如果文件顶部没有导入 bbox_iou 和 bbox2dist，请保留原有的导入
# from ultralytics.utils.metrics import bbox_iou
# from ultralytics.utils.ops import bbox2dist

def compute_nwd(pred_bboxes, target_bboxes, constant=12.8, eps=1e-7):
    """
    计算 xyxy 格式边界框的 Normalized Wasserstein Distance (NWD)。
    返回范围在 0~1 之间的张量，类似于 IoU。
    """
    # 1. 提取宽高和中心点 (xyxy -> cx, cy, w, h)
    w1 = pred_bboxes[:, 2] - pred_bboxes[:, 0]
    h1 = pred_bboxes[:, 3] - pred_bboxes[:, 1]
    cx1 = pred_bboxes[:, 0] + w1 / 2.0
    cy1 = pred_bboxes[:, 1] + h1 / 2.0

    w2 = target_bboxes[:, 2] - target_bboxes[:, 0]
    h2 = target_bboxes[:, 3] - target_bboxes[:, 1]
    cx2 = target_bboxes[:, 0] + w2 / 2.0
    cy2 = target_bboxes[:, 1] + h2 / 2.0

    # 2. 计算 Wasserstein 距离的平方
    center_dist = (cx1 - cx2) ** 2 + (cy1 - cy2) ** 2
    wh_dist = ((w1 - w2) ** 2 + (h1 - h2) ** 2) / 4.0
    w2_dist = center_dist + wh_dist

    # 3. 归一化为 NWD 指标 (指数映射到 0~1)
    nwd = torch.exp(-torch.sqrt(w2_dist + eps) / constant)
    return nwd
...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        """Compute combined IoU-NWD and DFL losses for bounding boxes."""
        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)
        ...
        # 3. 计算 NWD
        nwd = compute_nwd(pb, tb)
        
        # 强制对齐形状 (防止版本差异导致的广播错误)
        nwd = nwd.view_as(iou)

        # 4. 指标加权融合 (0.5 CIoU + 0.5 NWD)
        # combined_metric 越接近 1 越好
        combined_metric = self.alpha * iou + (1.0 - self.alpha) * nwd
        ...
        # 注意：这里返回的 loss_iou 实际上已经是融合了 NWD 的 loss_box
        return loss_iou, loss_dfl
...
class v8OBBLoss(v8DetectionLoss):
    ...
    def __init__(self, model, tal_topk=10, tal_topk2: int | None = None):
        ...
        # [Optimization] VFL for classification loss
        self.vfl = VarifocalLoss(gamma=2.0, alpha=0.75)

        # [Optimization] Dynamic alpha annealing for NWD+ProbIoU
        self.current_epoch = 0
        self.total_epochs = model.args.epochs if hasattr(model.args, 'epochs') else 100
    ...
    def loss(self, preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        对齐论文 §3.4 式 (26):
            L_total = λ_box · L_ProbIoU + λ_cls · L_Cls + λ_vib · L_VIB
        其中 λ_box=7.5 (来自 hyp.box), λ_cls=0.5 (来自 hyp.cls), λ_vib=0.01 (hyp.vib).
        保留 4-slot 张量 [box, cls, dfl, angle] 以兼容训练器日志,
        DFL / angle 两项置 0 (论文不引入), VIB 并入 box 汇总.
        """
        loss = torch.zeros(4, device=self.device)  # [box(含VIB), cls, dfl=0, angle=0]
        ...
        # 论文式 (26) L_Cls: 二值交叉熵, 不使用 VFL / label smoothing
        with autocast(enabled=False):
            loss[1] = F.binary_cross_entropy_with_logits(
                pred_scores.float(), target_scores.to(dtype).float(), reduction="sum"
            ) / target_scores_sum

        # 论文式 (22)~(25) L_ProbIoU: 纯高斯旋转框回归
        if fg_mask.sum():
            target_bboxes[..., :4] /= stride_tensor
            loss_probiou, _loss_dfl_unused = self.bbox_loss(
                pred_distri,
                pred_bboxes,
                anchor_points,
                target_bboxes,
            ...
            loss[0] = loss_probiou
            # DFL / angle 均不参与论文式 (26), 保留 0 占位
            # pred_angle 的梯度通过 bbox_decode → ProbIoU 自动回流, 无需独立角度项
        else:
            loss[0] += (pred_angle * 0).sum()

        loss[0] *= self.hyp.box  # λ_box = 7.5
        loss[1] *= self.hyp.cls  # λ_cls = 0.5
        # loss[2] (dfl) / loss[3] (angle) 在论文式 (26) 中均不出现, 维持 0

        # 论文式 (26) L_VIB: 所有 SIAGA 模块的熵控 KL 汇总, 统一施加 λ_vib
        vib_loss_sum = 0.0
        if hasattr(self, 'model'):
            for m in self.model.modules():
                if hasattr(m, 'vib_loss'):
                    vib_loss_sum = vib_loss_sum + m.vib_loss
        lambda_vib = getattr(self.hyp, 'vib', 0.01)
        loss[0] = loss[0] + lambda_vib * vib_loss_sum

        return loss * batch_size, loss.detach()  # loss(box+vib, cls, dfl=0, angle=0)
...
class E2ELoss:
    ...
    def current_epoch(self):
        return self.one2many.current_epoch if hasattr(self.one2many, "current_epoch") else 0

    @current_epoch.setter
    def current_epoch(self, epoch):
        if hasattr(self.one2many, "current_epoch"):
            self.one2many.current_epoch = epoch
        if hasattr(self.one2one, "current_epoch"):
            self.one2one.current_epoch = epoch
