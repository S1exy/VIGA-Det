...
def soft_nms_rotated(boxes, scores, iou_threshold=0.5, sigma=0.5,
                     score_threshold=0.001, iou_func=batch_probiou):
    """
    Soft-NMS for rotated (OBB) boxes using Gaussian score decay.
    Instead of hard-killing overlapping boxes, decays their scores proportionally to IoU.
    Particularly effective for dense parking scenes with closely packed vehicles.

    Args:
        boxes (torch.Tensor): Rotated bounding boxes with shape (N, 5) in xywhr format.
        scores (torch.Tensor): Confidence scores with shape (N,).
        iou_threshold (float): Not used for hard suppression; kept for API compatibility.
        sigma (float): Gaussian decay parameter. Smaller = more aggressive suppression.
        score_threshold (float): Minimum score to keep a detection.
        iou_func (callable): Function to compute IoU between rotated boxes.
    ...
    # Work on cloned scores to avoid modifying original tensor unexpectedly
    scores = scores.clone()
    keep = []
    ...
    while idxs.numel() > 0:
        ...
        rest = idxs[1:]
        ious = iou_func(boxes[i:i+1], boxes[rest]).squeeze(0)

        # Gaussian decay: score *= exp(-iou² / sigma)
        decay = torch.exp(-(ious ** 2) / sigma)
        scores[rest] *= decay

        # Filter out low-score boxes
        mask = scores[rest] > score_threshold
        idxs = rest[mask]
    ...
    return torch.stack(keep) if keep else torch.empty(0, dtype=torch.int64, device=boxes.device)
...
def non_max_suppression(
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        ...
        if rotated:
            ...
            # OBB NMS computes N×N probiou matrix in GPU memory.
            # Cap candidates to avoid OOM (8000² × 4B = 256MB, safe for 24GB GPU)
            max_nms_obb = 8000
            if boxes.shape[0] > max_nms_obb:
                topk_idx = scores.argsort(descending=True)[:max_nms_obb]
                boxes, scores = boxes[topk_idx], scores[topk_idx]
                x = x[topk_idx]  # keep x in sync for downstream indexing
                if return_idxs:
                    xk = xk[topk_idx]
